...
for a in serial_cps:
    serial_cpsX.append(a[0])
    serial_cpsY.append(a[1])
...
@compute @workgroup_size({uResultLength})
...
{{
    ...
    {{
        ...
        output[index] = f32(tempCps[tempIndex].x);
        output[index + {uResultLength}] = f32(tempCps[tempIndex].y);
    }}
...
# print(bSplineList)
# print("")
outLength = uResultLength*2
out = compute_with_buffers({0: uDraws, 1: vDraws, 2: serial_cps, 3: knots, 4: uIntervals, 5: vIntervals}, 
                           {6: (outLength, "f")}, compute_shader_code, n=1)
result = np.frombuffer(out[6], dtype=np.float32)

bSplineList = np.array([[result[a], result[a + uResultLength]] for a in range(0, uResultLength)])
print(bSplineList)
