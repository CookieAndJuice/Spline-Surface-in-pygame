...
def compute_bezier_points(vertices, numPoints=None):
    if numPoints is None:   # default 값이라는 소린가?
        numPoints = 30
...
def main():
    ...
    while running:
        ...
        ### Draw control "lines"
        pygame.draw.aalines(screen, lightgray, False, [(x.x, x.y) for x in control_points])

        ### Draw bezier curve
        b_points = compute_bezier_points([(x.x, x.y) for x in control_points], 30)
        pygame.draw.aalines(screen, pygame.Color("red"), False, b_points)
